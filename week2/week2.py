...
def func2(ss, start, end, criteria): 
    ...
    if ">=" in criteria:
        ...
        n=p[1]
    result=[]
    order=set(range(start, end+1))
    time=[]
    time.append(set(reserves1)&order)
    time.append(set(reserves2)&order)
    time.append(set(reserves3)&order)  
    
    if p[0] == "c" and z==">=" and n<=1200:
        i=0
        answer = None
        for s in services:
            if s["c"]>=n and len(time[i]) == 0:
                result.append(s["c"])
            i+=1 
        if len(result)==0:
            return print("Sorry") 
        ...
        if n =="S1" and len(time[0]) == 0:
            reserves1.extend(order)
            print("S1")
        elif n =="S2" and len(time[1]) == 0:
            reserves2.extend(order)
            print("S2")
        elif n =="S3" and len(time[2]) == 0:
            reserves3.extend(order)
            print("S3")
        else :
            print("Sorry")  
